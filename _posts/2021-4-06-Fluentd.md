...
+ 3.1 Write to files
 + Cons:  
  + Difficult to analyze by viewing the log files
  + Different format
 
+ 3.2 Log directly into Database
 + Cons:
  + Each application must be configured with DB connection
 
+ 3.3 Fluentd - Unified logging layer
 + Pros: 
  + Collect logs from different data sources (e.g. app log, system log, database log, access log)
  + The source can be in different formats
  + Enrich the log content by specifying the pod name, namespace, container name
  + Send to destination (e.g. Elastic Search, MongoDB, Kafka)
  + Support built-in reliability - Save data on HDD until the porcessing data is sent to the configured output destination 
  + i.e. Data still exists after restart and no additional storage is required
  + Auto retry sending the processing data to output destination
  + Support clustering to provide HA
